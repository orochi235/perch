...
func watchFromRaw(name string, raw rawWatch) (Watch, error) {
	...
	switch len(kinds) {
	case 0:
		return w, fmt.Errorf("watch.%s: needs one of run, http or exists", name)
	case 1:
		return w, nil
	default:
		return w, fmt.Errorf("watch.%s: has %v; a watch takes exactly one of run, http or exists", name, kinds)
	}
}
